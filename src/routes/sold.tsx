...
export const Route = createFileRoute("/sold")({
  head: () => ({ meta: [{ title: "Sold Listings" }, { name: "description", content: "Recently sold properties." }] }),
  component: Page,
});
...
function Inner() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["listings-sold", r?.realtor?.id],
    queryFn: () => fn({ data: { realtorId: r!.realtor!.id, status: "sold" } }),
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-20">
      ...
      <h1 className="mt-2 font-display text-5xl md:text-6xl">Sold</h1>
      ...
      {isLoading ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {data!.map((l) => <ListingCard key={l.id} listing={l} soldBadge />)}
        </div>
      )}
    </div>
  );
